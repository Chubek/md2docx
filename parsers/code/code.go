...
func ParseInlineCode(text string, para document.Paragraph, _ *document.Document, _ int) int {
	run := para.AddRun()
...
func ParseCodeBlock(text string, para document.Paragraph, doc *document.Document, _ int) int {
	text = text[4 : len(text)-4]

	style := doc.Styles
	customStyle := style.ParseStyle("CustomStyle1", wml.ST_StyleTypeParagraph, false)
